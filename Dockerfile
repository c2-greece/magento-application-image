...
RUN pecl install -o -f memcache && pecl install -o -f redis && rm -rf /tmp/pear &&  docker-php-ext-enable memcache &&  docker-php-ext-enable redis  &&  docker-php-ext-enable xsl
